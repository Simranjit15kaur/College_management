...
if (isset($_SESSION['ID'])) {
    ...
    $sql = "SELECT * FROM Professor WHERE P_ID = '$ID'";
    $sql1 = "SELECT * FROM Professor_email WHERE P_ID = '$ID'";
    $sql2 = "SELECT * FROM Professor_mobile WHERE P_ID = '$ID'";
    $result = $conn->query($sql);
    $result1=$conn->query($sql1);
    $result2=$conn->query($sql2);

    // Check if any rows were returned
    if ($result->num_rows > 0 and $result1->num_rows > 0 and  $result2->num_rows > 0) {
        ...
        echo "<p><strong>Professor ID:</strong> " . $row["P_ID"]. "</p>";
        echo "<p><strong>First Name:</strong> " . $row["First_Name"]. "</p>";
        echo "<p><strong>Last Name:</strong> " . $row["Last_Name"]. "</p>";
        echo "<p><strong>Date of Joining:</strong> " . $row["P_DOJ"]. "</p>";
        echo "<p><strong>Date of Birth:</strong> " . $row["P_DOB"]. "</p>";
        echo "<p><strong>Password:</strong> " . $row["P_Password"]. "</p>";
        echo "<p><strong>Role Title:</strong> " . $row["Role_Title"]. "</p>";
        ...
        echo "<p><strong>Email:</strong> " . $row1["P_Email"]. "</p>";
        echo "<p><strong>Mob No:</strong> " . $row2["P_Mobile"]. "</p>";
        echo "</div>";
        $conn->close();
...
?>
<a href="update_prof.php"><button>Update Profile</button></a>
    </div>
</body>
</html>
